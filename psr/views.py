...
class ImportGRRDataAPIView(APIView):
    permission_classes = [IsAuthenticated]
    parser_classes = (MultiPartParser, FormParser)

    def post(self, request):
        # 1. Validation
        # Assuming you're using a simple serializer with 'file' and 'dry_run' fields
        file_obj = request.FILES.get("file")
        dry_run = request.data.get("dry_run") == "true" or request.data.get("dry_run") is True

        if not file_obj:
            return Response({"detail": "No file uploaded"}, status=status.HTTP_400_BAD_REQUEST)

        # 2. Save to Temp File (to allow pandas to read via path)
...
# 3. Read Excel Content
            ...
            if df is None or df.empty:
                return Response({"detail": "Could not read Excel or sheet is empty"}, status=status.HTTP_400_BAD_REQUEST)

            # --- ADD THESE PRINT STATEMENTS HERE ---
            print("--- DEBUG: EXCEL COLUMNS DETECTED ---")
            ...
            return Response({
                ...
                "imported_new_records": len(entries),
                "duration_seconds": round(duration, 2)
            }, status=status.HTTP_201_CREATED)
...
class ImportPCRDataAPIView(APIView):
    permission_classes = [IsAuthenticated]
    parser_classes = (MultiPartParser, FormParser)
    ...
    def post(self, request):
        ...
        # 2. Save Uploaded File to Temporary Storage
        suffix = os.path.splitext(file_obj.name)[1]
        with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as tmp:
            for chunk in file_obj.chunks():
                tmp.write(chunk)
        ...
        try:
            ...
            # 4. PCR-Specific Column Validation
            required_cols = [
            ...
            if dry_run:
                return Response({
                    "dry_run": True,
                    "valid_rows_to_import": len(entries),
                    "total_rows_found": len(df)
                })

            # 7. Perform Atomic Database Update
            start_time = timezone.now()
            with transaction.atomic():
                # Wipe old dump data
...
class ProjectUpdateView(APIView):
    ...
    def patch(self, request, co_no):
        ...
        if has_psr_permission(user, "ADMIN"):
            ...
            return Response({"detail": "Updated directly"})
        ...
        return Response({"detail": "Update request submitted."})

    def _apply_component_updates(self, project, data):
        """
        Updates existing departments and sub-departments instead of creating them.
        """
        dept_budgets = data.get("department_budgets", {})
        sub_budgets = data.get("sub_department_budgets", {})
        cost_cat_budgets = data.get("cost_category_budgets", {})

        # 1. Update Departments Hourly Rates
        for dept_name, payload in dept_budgets.items():
            PSRDepartment.objects.filter(project=project, name=dept_name).update(
                hourly_rate=Decimal(str(payload.get("hourly_rate", "2000.00")))
            )

        # 2. Update Sub-Department Budgets
        for code, cost_value in sub_budgets.items():
            sub = PSRSubDepartment.objects.filter(department__project=project, code=code).first()
            if sub:
                sub.budget_cost = Decimal(str(cost_value))
                # Recalculate hours based on (possibly new) hourly rate
                rate = sub.department.hourly_rate
                sub.budget_hours = sub.budget_cost / rate if rate > 0 else Decimal("0")
                sub.save()

        # 3. Update Cost Categories
        for code, cost_value in cost_cat_budgets.items():
            ProjectCostCategory.objects.filter(project=project, cost_category__code=code).update(
                budget_cost=Decimal(str(cost_value))
            )

        # 4. Trigger Rollups & Snapshot
        for dept in project.departments.all():
    ...
    def _apply_project_updates(self, project, update_data):
        """
        Maps JSON data fields to PSRProject model fields and saves.
        """
        # List of fields to sync from the JSON 'data' to the Model
        fields_to_map = [
        ...
        for field in fields_to_map:
            if field in update_data:
                ...
                # Handle Decimal conversions for numeric fields
                if field in ['sales_value_foreign_curr', 'exchange_rate', 'direct_margin_percentage', 'risk']:
                    setattr(project, field, Decimal(str(value)) if value else Decimal('0'))
                else:
                    setattr(project, field, value)

        # Handle Foreign Key for Project Manager separately
        ...
        # The save() call on the model will trigger your custom save() logic 
        # which calculates EBIT, SK_Value, etc.
        project.save()
...
class AllProjectsLatestSnapshotView(APIView):
    ...
    def get(self, request):
        # 1. Start with the base filter (projects with snapshots)
        queryset = PSRProject.objects.filter(psr_snapshots__isnull=False)

        # 2. Check for the "Archived" query parameter
        # Usage: /api/url/?Archived=true (or any value)
        is_archived_requested = request.query_params.get('Archived')

        if is_archived_requested:
            # Only show Closed projects
            queryset = queryset.filter(current_phase="Closed")
        else:            # Exclude Closed projects (show only active)
            queryset = queryset.exclude(current_phase="Closed")
        # distinct() to avoid duplicates from the snapshots join
        projects_with_snapshots = queryset.distinct()
...
class PSRProjectCreationWorkflowView(APIView):
    ...
    def _handle_admin_stage(self, request, creation_request, action, comment):
        ...
        if action == "REJECT":
            ...
            # --- SYNC DATA FROM REQUEST TO PROJECT TABLE ---
            from .views import ProjectUpdateView, ProjectCreateView
            
            # 1. Update the Project fields (Name, Sales Value, Exchange Rate, etc.)
            updater = ProjectUpdateView()
            updater._apply_project_updates(creation_request.project, creation_request.data)
            updater._apply_component_updates(creation_request.project, creation_request.data)

            # 2. Update/Create the sub-components
            # If departments exist, you might want an '_update_project_components' 
            # but for now, we trigger the creation logic:
            if not creation_request.project.departments.exists():
                ProjectCreateView()._create_project_components(creation_request.project, creation_request.data)
...
class ProjectPhaseUpdateView(APIView):
    def patch(self, request, co_no):
        project = get_object_or_404(PSRProject, co_no=co_no)
        new_phase = request.data.get('current_phase')

        valid_phases = ["Open", "Closed"]
        
        if not new_phase:
            return Response(
                {"error": "current_phase field is required."}, 
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        # 4. Update the field and save
        project.current_phase = new_phase
        project.save() # This triggers your custom save() logic in models.py

        # 5. Return a manual dictionary response
        return Response({
