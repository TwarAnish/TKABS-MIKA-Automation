...
class GRRData(models.Model):
    # Critical fields (must be imported)
    co_no = models.CharField(max_length=20, db_index=True, help_text="CONo")
    grr_no = models.CharField(max_length=100, db_index=True, help_text="GRRNo")
    # Highly useful additional fields from the dump
    po_no = models.CharField(max_length=50, blank=True, help_text="PoNo")
    grr_date = models.DateField(null=True, blank=True, help_text="Po.Date")
    item_code = models.CharField(max_length=100, blank=True, help_text="ItemCode")
    description = models.TextField(blank=True, help_text="Description")
    supplier_name = models.CharField(max_length=255, blank=True, help_text="SupplierName")
    rate = models.DecimalField(max_digits=18, decimal_places=2, null=True, blank=True, help_text="Rate")
    received_qty = models.DecimalField(max_digits=18, decimal_places=2, null=True, blank=True, help_text="ReceivedQty")
    imported_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['co_no']),
            models.Index(fields=['grr_no']),
            models.Index(fields=['co_no', 'grr_no']),
        ]
        # constraints = [
        #     models.UniqueConstraint(
        #         fields=['co_no', 'po_no', 'sr_no'],
...
class PCRData(models.Model):
    # Critical fields (must be imported)
    co_no = models.CharField(max_length=20, db_index=True, help_text="CONo")
    pcr_no = models.CharField(max_length=100, db_index=True, help_text="PCRNo")
    # Highly useful additional fields from the dump
    po_no = models.CharField(max_length=50, blank=True, help_text="PoNo")
    pcr_date = models.DateField(null=True, blank=True, help_text="Po.Date")
    item_code = models.CharField(max_length=100, blank=True, help_text="ItemCode")
    description = models.TextField(blank=True, help_text="Description")
    supplier_name = models.CharField(max_length=255, blank=True, help_text="SupplierName")
    rate = models.DecimalField(max_digits=18, decimal_places=2, null=True, blank=True, help_text="Rate")
    recp_qty = models.DecimalField(max_digits=18, decimal_places=2, null=True, blank=True, help_text="ReceivedQty")
    imported_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['co_no']),
            models.Index(fields=['pcr_no']),
            models.Index(fields=['co_no', 'pcr_no']),
        ]
        # constraints = [
        #     models.UniqueConstraint(
        #         fields=['co_no', 'po_no', 'sr_no'],
...
class PSRSnapshot(models.Model):
    ...
    total_grr = models.DecimalField(max_digits=20, decimal_places=2, default=0)      # labor + material actual
    total_pcr = models.DecimalField(max_digits=20, decimal_places=2, default=0)      # labor + material actual
    poc = models.DecimalField(max_digits=20, decimal_places=2, default=0)            # percentage of completion based on actuals
