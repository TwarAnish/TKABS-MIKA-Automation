...
class ProjectBasicSerializer(serializers.ModelSerializer):
    ...
    def get_current_phase(self, obj):
        return obj.current_phase if obj.current_phase else "Open"
...
class PSRProjectCreationRequestSerializer(serializers.ModelSerializer):

    approval_progress = serializers.SerializerMethodField()
    project_details = serializers.SerializerMethodField()

    class Meta:
        model = PSRProjectCreationRequest
        fields = '__all__'

    def get_project_details(self, obj):
        return {
            "co_no": obj.project.co_no,
            "project_name": obj.project.project_name,
            "sales_value": obj.project.sales_value,
        }

    def get_approval_progress(self, obj):

        if obj.status == 'PENDING_APPROVERS':
            approvals = obj.approval_actions.filter(
                stage='APPROVER',
                action='APPROVE'
            ).count()
            return f"{approvals}/2 Approvers approved"
